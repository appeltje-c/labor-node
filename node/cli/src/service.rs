...
use labor_node_runtime::RuntimeApi;
use sc_service::{
...
use labor_node_executor::Executor;
...
mod tests {
	...
	use labor_node_runtime::{BalancesCall, Call, UncheckedExtrinsic, Address};
	use labor_node_runtime::constants::{currency::CENTS, time::SLOT_DURATION};
	use codec::Encode;
	use sp_core::{
	...
	#[test]
	// It is "ignored", but the labor-node ignored tests are running on the CI.
	// This can be run locally with `cargo test --release -p labor-node test_sync -- --ignored`.
	#[ignore]
	fn test_sync() {
